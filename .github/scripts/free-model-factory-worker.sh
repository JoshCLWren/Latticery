...
install_factory_rest_gh() {
  ...
  shim_dir="$(mktemp -d /tmp/latticery-factory-gh.XXXXXX)"
...
stage_trusted_kilo_helper() {
  ...
  if [[ -z "$TRUSTED_KILO_HELPER" ]]; then
    TRUSTED_KILO_HELPER="$(mktemp /tmp/latticery-kilo-auto-run.XXXXXX.sh)"
  fi
...
stage_trusted_review_controller() {
  local trusted_dir
  trusted_dir="$(mktemp -d /tmp/latticery-review-controller.XXXXXX)"
