...
stage_trusted_guard() {
  ...
  if [[ -z "$TRUSTED_GUARD" ]]; then
    TRUSTED_GUARD="$(mktemp /tmp/latticery-fixed-model-guard.XXXXXX.py)"
  fi
...
current_head_review_blockers() {
  ...
  [[ "$changes" == "0" ]] || return 1
  unresolved="$(gh api graphql -F owner="${GITHUB_REPOSITORY%%/*}" -F name="${GITHUB_REPOSITORY#*/}" -F number="$pr" \
...
is_model_cooldown_failure() {
  grep -Eiq 'cooling down|model_cooldown' "/tmp/opencode-factory-${WORKER}.log"
}
...
run_agent() {
  ...
  timeout --signal=TERM --kill-after=30s "${timeout_seconds}s" \
    ...
    --title "Latticery Factory ${WORKER} · ${DISPLAY}" "$prompt" \
