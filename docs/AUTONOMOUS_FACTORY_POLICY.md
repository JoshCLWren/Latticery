# the host repository Autonomous Factory Policy
...
Scheduled ChatGPT workers must update their assigned permanent comment on the configured Factory registry issue at the start and completion of every run, following `docs/FACTORY_GITHUB_VISIBILITY.md`. The registry and the watchdog's alert issue are operational telemetry, not executable backlog work. Workers must never claim, implement, label, or close them.
